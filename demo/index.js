...
function infoTemplate(params) {
  ...
  if (params.empty) {
    ...
    if (params.name === "status_disconnected") {
      text =
        "You are not connected. Please set the required fields and connect first.";
    }
...
$("#quick_call").on("click", function () {
  ...
  tlr.quickLinks.createLink(invited_person_name).then(function(data){    
    var localtime = new Date(data.expiration_time).toString().split(" GMT")[0];
...
$("#confroom_btn").on("click", function () {
  if (tlr.connected) {
    if (tlr.config.credentials.client_id === "syldemo") {
      tlr.notify({
        icon: "warning",
        icon_color: "red",
        message:
          "You can't create conference room with the demo account. Contact us in order to get a valid account.",
        autohide: true,
      });
    } else {
      tlr.confRoom.open();
    ...
    tlr.notify({
      ...
      icon_color: "red",
      message:
        "You have to be connected in order to create the conference room",
      autohide: true,
    });
  }
});
...
$("#open_confrooms_list").on("click", function () {
  if (!tlr.connected) {
    tlr.notify({
      ...
      icon_color: "red",
      message:
        "You need to be connected in order to view list of conference rooms.",
      autohide: true,
    });
  } else {
    tlr.confRoom.openList();
  }
});
...
$("#call_phone_number_btn").on("click", function () {
  if (tlr.config.credentials.client_id === "syldemo") {
    tlr.notify({
      ...
      icon_color: "red",
      message:
        "You can't call phone numbers with the demo account. Contact us in order to get a valid account.",
      autohide: true,
    });
  } else if (!tlr.connected) {
    tlr.notify({
      icon: "warning",
      icon_color: "red",
...
$("#send_sms_btn").on("click", function () {
  if (tlr.config.credentials.client_id === "syldemo") {
    tlr.notify({
      ...
      icon_color: "red",
      message:
        "You can't send sms messages with the demo account. Contact us in order to get a valid account.",
      autohide: true,
    });
  } else if (!tlr.connected) {
    tlr.notify({
